...
class LoginView(View):
    ...
    def post(self,request):
        ...
        if login_form.is_valid():
            user_name = request.POST.get("username", "")  # 第二个参数为默认值
    ...
    def post(self,request):
        ...
        if register_form.is_valid():
            ...
            if UserProfile.objects.filter(email=user_name):
                return render(request, "register.html",{"register_form": register_form,"msg": u"该用户已存在！"})
...
class ForgetPwdView(View):
    ...
    def post(self,request):
        ...
        if forget_form.is_valid():
            ...
            url = send_register_email(email,"forget")
            return render(request,"send_success.html",{"url":url})
        else:
            return render(request,"forgetpwd.html",{"forget_form":forget_form})
...
class ModifyPwdView(View):
    ...
    def post(self,request):
        ...
        if modify_form.is_valid():
            ...
            return render(request,"login.html",{"msg":u"密码修改成功，请重新登录"})
        else:
            email = request.POST.get("email", "")
...
class SendEmailCodeView(LoginRequiredMixin,View):
    ...
    def post(self,request):
        ...
        success = json.dumps({"email":[send_register_email(email,'update_email')]})
...
class IndexView(View):
    ...
    """
    def get(self,request):
        all_bannners = Banner.objects.all().order_by('index')
        courses = Course.objects.filter(is_banner=False)[:6]
        banner_courses = Course.objects.filter(is_banner=True)[:3]
        course_orgs = CourseOrg.objects.all()[:15]

        return render(request,'index.html',{
            'all_bannners':all_bannners,
            'courses':courses,
